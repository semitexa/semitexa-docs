...
If you need the public URL shape to be editable per environment without changing PHP code, see [PAYLOAD_ENV_ROUTE_OVERRIDES.md](PAYLOAD_ENV_ROUTE_OVERRIDES.md). `#[AsPublicPayload(path: 'env::VAR::/fallback')]` is the canonical pattern.
