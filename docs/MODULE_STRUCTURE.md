...
One file is permitted directly at the code root — that is `packages/semitexa-{name}/src/Capabilities.php`, not the package root — and every Composer package is **required** to carry it. The class is `Capabilities` in the package's root namespace, so `semitexa/ssr` declares `Semitexa\Ssr\Capabilities`; the guard resolves it from the file's own `namespace` line, so the name has to match what the package's PSR-4 prefix maps `src/` to. It holds no code — only `#[Capability]` declarations describing what the package offers, which is how a project that has *not* installed the package can still learn it exists. The name and the position are both fixed, because the catalog guard checks exactly this class; a description filed away under `Application/` or `Domain/` would be the package's own summary hidden inside its implementation. See § 2 of `AGENTS.md` on `ai:ask mechanisms`.

Required, not encouraged: `ai:verify` plans a `capability_coverage` target whenever a package manifest or a declaration file changes, and fails naming each package that ships without one. Packages that also ship attributes still declare here — a mechanism says what you *write*, the package entry says what you *install*, and a reader who has neither sees ten attributes and no sentence telling them which package to require. The two degenerate cases (`semitexa/core`, which cannot be missed; `semitexa/ultimate`, which you are already inside) declare anyway, so that a missing entry always means an oversight and never a decision the reader has to reconstruct. A package with genuinely nothing missable to offer goes in `CapabilityIndex::DECLARATION_EXEMPT` with its reason, not in silence. Directories with no `composer.json` — a browser extension, the installer scaffold — are outside the rule by what they are: nothing can require them, so there is nothing to advertise.

The shipped index is built from the packages **on disk**, not from what the building checkout happens to have installed. The two are not the same set, and the difference runs the wrong way: the packages no working copy requires are precisely the ones a consumer most needs the index to tell them about.
...
Application modules and packages share the same envelope: runtime PHP under `src/`, tests under `tests/`, and module-owned **non-PHP assets** under `resources/` — the slot `semitexa/prompt` points its default prompt-body path at (`resources/prompts/{id}.twig`, resolved relative to the owning package or module root). Local module roots accept only `src/`, `tests/`, `resources/`, and a small set of metadata files (`composer.json`, `.gitkeep`, `README.md`); anything else fires `module_structure.unknown_directory` (or `invalid_layer` for package-only layer names like `Auth/`, `Attribute/`, `Discovery/`, `OpenApi/`, `Pipeline/`). Like a package's, a module's `resources/` holds assets, not code — the validator does not walk it.
...
`Application/Static/` classifies its children by asset **type** — `css`, `js`,
`img`, `fonts`, `svg`, `audio`. A self-hosted third-party bundle cannot honour
that, because it is one indivisible artifact holding several types at once with
internal relative references between them: Leaflet ships `leaflet.js`,
`leaflet.css` and `images/*.png` together, and the CSS references those images
relative to itself. Splitting it across the type directories breaks the vendored
file's own URLs unless you edit third-party code the next upgrade overwrites;
putting the whole bundle under one type directory is a misfiling the runtime
never asked for, since `StaticAssetHandler` and `ModuleAssetResolver` both
dispatch on file extension and never on the directory a file sits in.
